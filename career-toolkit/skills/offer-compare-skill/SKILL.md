...
2. 按 [frameworks/offer-compare-report.md](frameworks/offer-compare-report.md) 的规格 + [examples/offer-compare-template.html](examples/offer-compare-template.html) 的骨架，组装 5 节报告（Header + Verdict + Gauges + §1-§5 + Footer）。
   - ⛔ **品牌 footer 是强制项，不是装饰。** 报告结尾必须**原样**包含 `<footer>` 里的 `OFFER COMPARE.` brand mark + `Created with Career Toolkit` 副标题 + LinkedIn / X / Toolkit三个社交按钮（含对应 CSS：`.brand-block` / `.brand-mark` / `.socials` / `.foot-meta`）。直接从 [frameworks/offer-compare-report.md](frameworks/offer-compare-report.md) 末尾「📌 强制 Footer 区块」整段抄过去。
