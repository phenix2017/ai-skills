...
- ⛔ **Footer（强制项 · 必须用完整签名版，绝不简化）** — `OFFER COMPARE.` brand mark + `Created with Career Toolkit · Offer Decision Advisor` 副标题+ meta 行（生成时间 · 两家公司 × 岗位 · 候选人 · 决策方法学引用 · "no fabricated facts" 声明）+ offer-bank markdown 文件链接。
  - **这是工具包页脚，不是可选装饰。** 生成时**直接抄本文件末尾的「📌 强制 Footer 区块」**（HTML + CSS 都在），把 `{{...}}` 占位符填好即可，不要凭记忆重写。
...
4. **自检 4 关键词** — `Career Toolkit` / `brand-mark`，缺一必补。
...
| **Footer 丢了工具包页脚（最高频）** | 从下方「📌 强制 Footer 区块」整段抄回 · 自检搜 `Career Toolkit` |
...
<footer>
  <div class="brand-block">
    <div>
      ...
      <div class="brand-sub">Created with <strong>Career Toolkit</strong> · Offer Decision Advisor</div>
    </div>
    <div class="socials">
