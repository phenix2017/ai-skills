...
**铁律**
- **必须先有 Decode 产出再算 Match。** 没解码就匹配 = 跟招聘话术对齐，不是跟真实需求对齐。如果库里没解码，先跑一版极简解码（至少出 Must Have + Hidden Signals）。
...
1. **JD 的解码产出**（从 `jd-bank/` 调取，或现场跑解码）。
...
| Decode 还没做 | 先跑一版极简解码（只出 Must Have + Hidden Signal）再来算 |
| 用户给的简历跟职位完全不沾边 | 直接说"这个匹配度太低（< 40%），可能不值得花精力 tailor，建议先看 Should I Apply 步骤 决定要不要投" |
